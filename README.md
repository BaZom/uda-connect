...
### Goal
The goal of this submission is refactor the udaconnect monolotic application (https://github.com/udacity/nd064-c2-message-passing-projects-starter) into a mircoservices applications using message passing techniques like REST-API, gRPC and kafka
...
#### Deployment Note
You may notice the odd port numbers being served to `localhost`. [By default, Kubernetes services are only exposed to one another in an internal network](https://kubernetes.io/docs/concepts/services-networking/service/). This means that `udaconnect-app` and `person-api` can talk to one another. For us to connect to the cluster as an "outsider", we need to a way to expose these services to `localhost`.
...
### Docker Images
All modules/services use docker images from `bazom/<image_name>`. To make changes to the application, build your own Docker image and push it to your own DockerHub repository. Replace the existing container registry path with your own.
